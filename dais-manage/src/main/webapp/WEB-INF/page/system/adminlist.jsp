    ...
    <form  >
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" id="name"  placeholder="请输入真实姓名"  class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">用户名</label>
            <div class="layui-input-block">
                <input type="text" id="username"  placeholder="请输用户名"  class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">登录密码</label>
            <div class="layui-input-block">
                <input type="text" id="password"  placeholder="请输入登录密码"  class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-inline">
                <label class="layui-form-label">选择权限</label>
                <select id="roleid" style="width: 160px;height: 33px;">
...
<script type="text/javascript">
    ...
    function getSysParamInfo(data){
        ...
        $.each(data,function(v,o){
            ...
            s+='<td style="display: none">'+o.roleid+'</td>';
            s+='<td >'+o.name+'</td>';
            s+='<td >'+o.username+'</td>';
            s+='<td >'+o.ip+'</td>';
            s+='<td>'+o.rolename+'</td>';
            s+='<td>'+o.createuser+'</td>';
            s+='<td>'+formatDate(o.logintime)+'</td>';
            s+=' <c:if test="${user_info.roleid < 3}"><td><button onclick="edit(this)" class="layui-btn layui-btn-small"><i class="layui-icon"></i></button>' +
                    '<button class="layui-btn layui-btn-small" onclick="del(this)"><i class="layui-icon"></i></button></td></c:if></tr>';
        });
        if(data.length>0){
            $("#pageDiv").show();
    ...
    function edit(othis){
        ...
        $("#name").val($(obj).eq(3).text());
        $("#username").val($(obj).eq(4).text());
        $("#roleid").val($(obj).eq(2).text());
    }
    function add(){
        layer.open({
            title: "新增用户"
...
//            ,id: 'system_dailog' //防止重复弹出
    ...
    function save(){
        ...
        if(name == ""){
            showMsgDailog("真实姓名不能为空");
            return false;
        }
        if( username== ""){
            showMsgDailog("用户名不能为空");
            return false;
        }
        if(id== "" && password == ""){
            showMsgDailog("密码不能为空");
            return false;
        }
