...
public class BTCUtils {
	...
	public JSONObject getbalance(String account) throws Exception {
		String s = main("getbalance", "[\""+account+"\",1]");
		JSONObject json = JSONObject.fromObject(s);
		return json;
	}

	public double getbalanceValue(String account) throws Exception {
		double result = 0.0000f;
		JSONObject s = getbalance(account);
		if(s.containsKey("result")){
			result =Double.valueOf(s.get("result").toString());
		}
		return result;
	...
	public String getaccountValue(String address) throws Exception {
		String result = "";
		JSONObject s = getaccount(address);
		if(s.containsKey("result")){
	...
	public JSONObject dumpprivkey(String address,boolean ispass) throws Exception {
		if (ispass){
			if(PASSWORD != null && PASSWORD.trim().length() >0){
				walletpassphrase(30);
			}
		}
		String s = main("dumpprivkey", "[\""+address+"\"]");
		JSONObject json = JSONObject.fromObject(s);
		if (ispass){
			if(PASSWORD != null && PASSWORD.trim().length() >0){
				walletlock();
	...
	public String getNewaddressValueForAdmin(String userId) throws Exception {
		String result = null;
		if(PASSWORD != null && PASSWORD.trim().length() >0){
			JSONObject json = walletpassphrase(30);
        ...
        if(json.containsKey("result") && !json.get("result").toString().equals("null")){
        	...
        	while(it.hasNext()){
        		...
        		if(map.get("category").toString().equals("receive")){
        			...
        			try {
						long time = Long.parseLong(map.get("time").toString()) * 1000;
	 ...
	 * 根据交易ID取得交易明细
	 * */
	public JSONObject gettransaction(String xid) throws Exception {
	...
	public BTCInfo gettransactionValue(String xid,String category) throws Exception {
        ...
        if(json.containsKey("result") && !json.get("result").toString().equals("null")){
			...
			try {
				...
				if(map.get("blockindex") != null && map.get("blockindex").toString().trim().length() >0){
					btcInfo.setBlockindex(Integer.parseInt(map.get("blockindex").toString()));
				}
				if(map.get("fee") != null && map.get("fee").toString().trim().length() >0){
					btcInfo.setFee(Double.valueOf(map.get("fee").toString()));
				}
			} catch (Exception e) {
	...
	public JSONObject sendfrom(String fromAccount, String bitcoinAddress,double amount,String comment) throws Exception {
		if(PASSWORD != null && PASSWORD.trim().length() >0){
			JSONObject json = walletpassphrase(30);
			if ("500".equals(json.getString("error"))) {
				System.out.println("error pass=" + PASSWORD + "!");
		...
		if(StringUtils.isEmpty(comment)){
			s = main("sendfrom", "[\""+fromAccount+"\",\""+bitcoinAddress+"\","+amount+"]");
		}else {
			s = main("sendfrom", "[\""+fromAccount+"\",\""+bitcoinAddress+"\","+amount+","+comment+"]");
		}
		if(PASSWORD != null && PASSWORD.trim().length() >0){
			walletlock();
		}
	...
	public JSONObject settxfee(double txfee) throws Exception{
		String s = main("settxfee", "["+txfee+"]");
		JSONObject json = JSONObject.fromObject(s);
		return json;
	}
	...
	public String sendfromValue(String fromAccount, String bitcoinAddress,double amount,String comment) throws Exception {
		String result = "";
		JSONObject s = sendfrom(fromAccount, bitcoinAddress,amount, comment);
		if(s.containsKey("result")){
			if(!s.get("result").toString().equals("null")){
				result = s.get("result").toString();
			}
	...
	//解锁
	public JSONObject walletpassphrase(int times) throws Exception {
		String ret = main("walletpassphrase","[\""+PASSWORD+"\","+times+"]");
