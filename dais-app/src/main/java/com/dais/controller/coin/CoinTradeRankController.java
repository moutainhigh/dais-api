...
import com.dais.mapper.FvirtualCoinDescMapper;
...
public class CoinTradeRankController extends BaseController {
    ...
    @Autowired
    private FvirtualCoinDescMapper fvirtualCoinDescMapper;
    ...
    @RequestMapping(value = "/getCoinDesc")
    @ResponseBody
    public ResultModel getList(String coinType) {
        try {
            FvirtualcointypeExample example1 = new FvirtualcointypeExample();
            example1.createCriteria().andFshortnameEqualTo(coinType);
            List<Fvirtualcointype> list1 = this.virtualCoinService.selectByExample(example1);
            if(CollectionUtils.isEmpty(list1)){
                return ResultModel.build(500, "暂无数据");
            }
            FvirtualCoinDescExample example = new FvirtualCoinDescExample();
            example.createCriteria().andSymbolEqualTo(list1.get(0).getFid());
            List<FvirtualCoinDesc> list = fvirtualCoinDescMapper.selectByExample(example);
